...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${useLightHeader ? 'bg-white shadow-md' : useSolidBackground ? 'bg-[#001f40] shadow-md' : 'bg-[#001f40] lg:bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <Link to="/" className={`font-medium transition-colors ${location.pathname === '/' ? 'text-blue-600' : useLightHeader ? 'text-gray-900 hover:text-blue-600' : 'text-gray-200 hover:text-white'}`}>Home</Link>
            <Link to="/platform" className={`transition-colors ${location.pathname === '/platform' ? 'text-blue-600' : useLightHeader ? 'text-gray-600 hover:text-blue-600' : 'text-gray-200 hover:text-white'}`}>Platform</Link>
            <Link to="/services" className={`transition-colors ${location.pathname === '/services' ? 'text-blue-600' : useLightHeader ? 'text-gray-600 hover:text-blue-600' : 'text-gray-200 hover:text-white'}`}>Services</Link>
          ...
          >
            <span className={`w-6 h-0.5 block transition-all ${(useLightHeader || menuOpen) ? 'bg-gray-900' : 'bg-white'} ${menuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
            <span className={`w-6 h-0.5 block transition-all ${(useLightHeader || menuOpen) ? 'bg-gray-900' : 'bg-white'} ${menuOpen ? 'opacity-0' : ''}`}></span>
            <span className={`w-6 h-0.5 block transition-all ${(useLightHeader || menuOpen) ? 'bg-gray-900' : 'bg-white'} ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
          </button>
        </div>
        ...
        {/* Mobile Navigation */}
        {menuOpen && (
          <div className="md:hidden absolute top-0 left-0 w-full bg-white shadow-xl transition-all duration-300 pt-20 pb-6 max-h-[100vh] overflow-y-auto">
