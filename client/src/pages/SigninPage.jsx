...
import { storeUserData, getStoredUserEmail, areCookiesAllowed } from "../utils/cookieUtils";
...
const SigninPage = () => {
  ...
  const handleGoogleAuth = () => {
    try {
      window.location.assign("/api/auth/google?mode=signin");
    } catch {
      setApiError("Google authentication is currently unavailable. Please try again.");
    }
  };

  // Load saved email from cookies on mount
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (googleStatus === "success" && email) {
      ...
      addToast("Google sign in successful!", "success", 3000);
      navigate("/");
    }
    ...
    if (!emailError && !passwordError) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          addToast("Successfully signed in!", "success", 3000);
          setTimeout(() => {
            navigate("/");
          }, 500);
        }
